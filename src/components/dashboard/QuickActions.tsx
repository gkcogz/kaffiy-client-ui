...
export const AnalyticsCard = () => {
  ...
  const totalVisits = 45; // Mock data
  const returningPercentage = 70;
  const newPercentage = 30;
  
  const chartData = chartPeriodData[period];
    ...
    <>
            ...
            <div className="px-4 pb-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                >
                  ...
                  <DailyVisitsChart period={period} />
                </div>
              </div>
            </div>
          </CollapsibleContent>
      ...
      <Dialog open={expandedChart === "distribution"} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {period === "daily" ? "Günlük" : period === "weekly" ? "Haftalık" : "Aylık"} Dağılım - Detaylı Analiz
            </DialogTitle>
          </DialogHeader>
...
export const QuickActions = () => {
  ...
  return (
    <>
      ...
      {/* Distribution Chart Modal */}
      <Dialog open={expandedChart === "distribution"} onOpenChange={(open) => !open && setExpandedChart(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Günlük Dağılım - Detaylı Analiz</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground uppercase tracking-wide mb-2">Toplam Ziyaret</p>
                <p className="text-2xl font-bold text-foreground">{dailyChartData.reduce((sum, d) => sum + d.visits, 0)}</p>
              </div>
              <div className="p-4 rounded-xl bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground uppercase tracking-wide mb-2">En Yoğun Zaman</p>
                <p className="text-lg font-bold text-foreground">2 PM</p>
                <p className="text-sm text-muted-foreground mt-1">11 ziyaret</p>
              </div>
            </div>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={dailyChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="dailyVisitGradientModal" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="hsl(72, 22%, 38%)" stopOpacity={0.4} />
                      <stop offset="100%" stopColor="hsl(72, 22%, 38%)" stopOpacity={0.05} />
                    </linearGradient>
                  </defs>
                  ...
                  <Area 
                    ...
                    stroke="hsl(72, 22%, 38%)"
                    strokeWidth={2}
                    fill="url(#dailyVisitGradientModal)"
                    dot={{ r: 4, fill: 'hsl(72, 22%, 38%)', strokeWidth: 2, stroke: '#fff' }}
                  />
                  <Bar 
                    dataKey="current"
                    fill="hsl(35, 30%, 55%)"
